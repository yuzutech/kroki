...
export class Worker {
  constructor() {
    this.pageUrl =
      process.env.KROKI_DIAGRAMSNET_PAGE_URL ||
  ...
  async _eval(page, task) {
    return await Promise.race([
      page
        .evaluate(
          async (source, unsafe) => {
            const resolveImage = async svg => {
              for (const img of await svg.querySelectorAll('image')) {
                if (img.attributes['xlink:href'].value.startsWith('data:')) {
                  continue
                }
                const imgBase64 = await fetch(img.attributes['xlink:href'].value).then(
                  async value => {
                    const mimeType = value.headers.get('content-type')
                    const b64img = btoa(
                      String.fromCharCode(...new Uint8Array(await value.arrayBuffer()))
                    )
